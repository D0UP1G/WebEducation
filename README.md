...
Получены [официальный брендбук и базовый учебный пакет](docs/organizer/README.md): три курса, девять модулей, 30 шагов. Учебное содержание ещё не перенесено в приложение. PR #34 перенёс токены брендбука в рабочий интерфейс; проверка всех ролей на стенде остаётся открытой. Текущая браузерная Python-сдача подходит для учебного демо, но не доказывает выполнение присланного кода. По новому требованию команды для защищённого зачёта планируется изолированный серверный запуск официальных сдач при локальной самопроверке в браузере. Актуальные задачи — в [плане четырёх агентов](docs/team-work-plan.md), расхождения — в [сверке](docs/current-gap-analysis.md).
