...
class Dev4ApiAcceptanceTests(TestCase):
    ...
    def test_publish_assign_review_progress_and_revision_pinning(self):
        ...
        updated_assignment = self.call(self.student, "get", enrollment_path, 200)
        self.assertEqual(updated_assignment["course_revision_id"], next_revision["id"])
        self.assertEqual(updated_assignment["title"], "Синтетический курс DEV-4 v2")
        self.assertEqual(updated_assignment["version"], 2)
        self.assertEqual(updated_assignment["progress"]["earned_points"], 5)
