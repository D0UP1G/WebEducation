...
удалён из Compose. Ниже перечислены реализованные функции и оставшаяся работа.
...
TypeScript/Vite build проходили; новых миграций не было. В локальном браузерном
стенде проверена часть сквозного пути: демо-ученик запустил Python на двух тестах через
...
## Открытые задачи после объединения

- **DEV-3:** сигнал `no_credit_72h` использует дату отправки принятой работы,
  хотя ручное принятие могло произойти позже. Считать время последнего зачёта
  от фактического решения куратора и проверить позднее принятие тестом.
- **DEV-3:** Python challenge живёт 600 секунд, но разрешены до 50 тестов по
  30 секунд. Согласовать срок токена и максимально допустимый прогон так,
  чтобы корректная сдача не истекала во время выполнения; добавить тест.
- **DEV-3:** `ReviewInput` требует поле `comment` и при `accepted`. Сделать его
  необязательным для принятия, сохранив обязательный непустой комментарий при
  возврате; покрыть оба случая тестом.
