...
Команда `python manage.py import_curriculum --owner admin_demo --publish`
повторно импортирует и публикует карту курсов; `seed_demo` готовит эти курсы и
создаёт синтетические аккаунты с чистыми назначениями, без тестового курса и
предзаполненных попыток.
