...
**Актуальный Python-контракт (`develop` после PR #50):** локальная самопроверка
...
браузерных результатов больше не действуют. PR #51 добавляет паспорт курса и
структуру модулей к ответам каталога; остальные контракты v1, включая составные
...
Курс, созданный из пакета организатора, хранит паспорт (`tool`, `goal`, `volume`)
и стабильный `source_id`. В `GET /student/enrollments/{enrollment_id}` поле
`modules` содержит упорядоченные модули; каждый модуль содержит `id`,
`source_id`, `position`, `title` и вложенные шаги. У опубликованной версии
`CourseRevisionSerializer` возвращает ту же иерархию. Поле `steps` на верхнем
уровне ответа enrollment сохраняет прежний плоский порядок для совместимости.
Контент каждого шага проходит тот же `public_step_content`, поэтому правильные
ответы, тесты и критерии куратора не становятся видимы ученику.

`import_curriculum` — Django management command, не публичный API. Администратор
стенда запускает `python manage.py import_curriculum --owner admin_demo --publish`;
повторный запуск по тем же `source_id` не создаёт новую ревизию, если карта не
изменилась. `seed_demo` также импортирует и публикует курсы для демонстрации.
