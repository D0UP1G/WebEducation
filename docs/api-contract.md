...
**Статус после получения материалов:** документ описывает действующий API на develop после PR #27. Для защищённого официального Python-зачёта текущий endpoint, принимающий результаты браузера, должен быть заменён или отделён от локальной самопроверки: официальный статус и баллы вычисляет только изолированный серверный исполнитель по присланному коду. Контракт модулей, составных ручных работ и нового Python-потока согласовать до реализации по [текущему плану](team-work-plan.md); не считать описанный ниже браузерный challenge защищённым зачётом.
...
Без явных требований шага валидатор принимает любой из этих наборов для обоих
типов. Один файл или одна ссылка сохраняют совместимость со старыми курсами.
...
Дополнение DEV-3 в ветке `feature/dev3-evidence-criteria`: `content.required_evidence`
...
[DEV-3 handoff](dev3-evidence-handoff.md). Ветка содержит обработку
`artifact.project` для интеграции с PR #35.
