...
**Статус после PR #35:** документ описывает действующий API на `develop`. Для `artifact.scratch`, `artifact.minecraft` и `artifact.project` сервер принимает файл, ссылку и пояснение в одной попытке; `scratch.numeric_answer` и `artifact.project` доступны в реестре типов. Требования к обязательным доказательствам и приватным критериям для конкретных шагов готовятся в PR #36. Для защищённого официального Python-зачёта текущий endpoint, принимающий результаты браузера, должен быть заменён или отделён от локальной самопроверки: официальный статус и баллы вычисляет только изолированный серверный исполнитель по присланному коду. Контракт модулей и нового Python-потока — в [текущем плане](team-work-plan.md); не считать описанный ниже браузерный challenge защищённым зачётом.
