...
Локальная самопроверка Python получает один открытый пример через
`GET .../python-sample`; официальная сдача принимает только
`{ "code": "..." }`. Код исполняется отдельным runner-контейнером, который
возвращает серверный статус. Составные ручные доказательства, паспорт курса,
модули, переписки, управление пользователями и защищённые предпросмотры входят
в текущий API.
...
синтетические; содержание трёх учебных курсов взято из пакета организатора.
...
`GET /student/enrollments/{enrollment_id}` возвращает для плоского списка `steps`
...
Запрос к закрытому шагу отвечает `404`, чтобы не раскрывать его содержимое.
Повторная отправка ранее зачтённой Python-задачи остаётся разрешена, пока все её
предшественники зачтены. Проверка действует на сервере, поэтому скрытие шага
только в интерфейсе не считается защитой.

### Структура курсов и модулей

Курс, созданный из пакета организатора, хранит паспорт (`tool`, `goal`, `volume`)
и стабильный `source_id`. В `GET /student/enrollments/{enrollment_id}` ответ
содержит диапазон классов (`grade_min`, `grade_max`) и поле `modules` с
упорядоченными модулями; каждый модуль содержит `id`,
`source_id`, `position`, `title` и вложенные метаданные шагов. У опубликованной версии
...
Детальное содержимое выдаёт отдельный endpoint только для открытого шага;
закрытые правильные ответы и тесты не попадают в enrollment.
...
изменилась. `seed_demo` импортирует и публикует только официальные курсы,
создаёт синтетические роли и чистые назначения без предзаполненных сдач.
...
`content.required_evidence` ручного шага задаёт обязательные поля одной попытки
...
очереди и карточке проверки. Проверка действует и для `artifact.project`.
...
выбирает первый незачтённый шаг без ожидающей проверки; ранее принятый шаг можно
открыть повторно для просмотра и новой Python-отправки. Публикация требует шаг `theory`, хотя бы
