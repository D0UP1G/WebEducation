...
**Статус после PR #30:** документ описывает действующий API на `develop`. Для `artifact.scratch` и `artifact.minecraft` сервер принимает файл, ссылку и пояснение в одной попытке; схема шага пока не задаёт обязательные доказательства для каждого задания. Для защищённого официального Python-зачёта текущий endpoint, принимающий результаты браузера, должен быть заменён или отделён от локальной самопроверки: официальный статус и баллы вычисляет только изолированный серверный исполнитель по присланному коду. Контракт модулей, типа «Проект», требований заданий и нового Python-потока — в [текущем плане](team-work-plan.md); не считать описанный ниже браузерный challenge защищённым зачётом.
...
Текущий валидатор принимает любой из этих наборов для обоих типов. Обязательные
сочетания доказательств для конкретного задания нужно определить в схеме шага
при переносе пакета; один файл или одна ссылка сохраняют совместимость со
старыми курсами. Повторная сдача после `incorrect`, `returned`
...
шаги повторно можно в любом порядке. Действующий код публикации требует шаг
`theory` и положительную сумму `max_score`. По решению владельца от 24.09
DEV-1 должен дополнить правило хотя бы одним контрольным вопросом
(`quiz.single_choice` или `quiz.multiple_choice`); до его PR остальные типы
технически необязательны.
