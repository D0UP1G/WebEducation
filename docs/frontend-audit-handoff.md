...
Исторический handoff DEV-2 до объединения PR #9 и #10. Описанные ниже
открытый PR, отсутствие API DEV-3 и задачи интеграции отражают прежний срез,
а не текущее состояние `develop`. Актуальные результаты — в
...
4. DEV-1 реализовал атомарную перестановку на существующем `PATCH step`:
   клиент передаёт только целевую `position`, сервер сдвигает шаги в одной
   транзакции. После обновления frontend перестановка выполняется одним запросом.
...
3. DEV-2 согласует точные JSON shapes, добавляет контекст задания куратору,
   переход к следующему шагу и подтверждение публикации.
