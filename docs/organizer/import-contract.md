...
Этот документ фиксирует подготовку одного шага из [карты курса](curriculum-map.json) в draft-представление приложения. Реализованы database-independent преобразователь плана, предварительная проверка `python manage.py preflight_curriculum_import` и транзакционный импорт `python manage.py import_curriculum --publish` из каталога `backend`. Импорт сверяет SHA-256 источника, валидирует все шаги, сохраняет паспорт и группировку модулей, а повторный запуск обновляет только записи с известными ключами и не создаёт дубликаты.
...
Тесты проверяют 3 курса / 9 модулей / 30 шагов, суммы 10/8/12 баллов, группировку ревизий, повторный импорт и неприкосновенность пользовательского курса. Официальный серверный Python-runner и стенд остаются отдельными задачами.
