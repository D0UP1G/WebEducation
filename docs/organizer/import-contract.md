...
4. Владелец согласует положительные баллы по шагам. Манифест намеренно не заполняет `max_score`; преобразователь сохраняет `null` для черновика, не подменяя его вымышленным значением.
