...
Для демонстрации команда готовит собственный синтетический курс и синтетические учётные записи. Полученный от организатора базовый учебный пакет нужно перенести в эту модель после проверки прав на публикацию материалов; официальный брендбук ещё ожидается. Текущее соответствие кейсу отражено в [сверке](docs/case-alignment.md).
...
- **Развёртывание:** Docker Compose на одном Linux-сервере: веб/API, PostgreSQL и обратный прокси. Python-код запускается в браузере ученика.
...
Всё приложение — один модульный Django-бэкенд. Ученический Python выполняется в Web Worker через Pyodide на устройстве ученика; Django сверяет присланный вывод с ожидаемыми ответами и назначает статус/баллы. Отдельные микросервисы для пользователей, курсов и прогресса на хакатоне не нужны.
...
Проверка Python-кода — обязательный сценарий кейса. API выдаёт подписанное задание с входами тестов и лимитами, привязанное к ученику, шагу и хешу кода. Web Worker запускает Python через Pyodide; сервер сверяет полученный вывод с ожидаемыми значениями и сохраняет попытку. Ожидаемые ответы остаются на сервере.
...
Контракт API, схема типов шагов, статусы сдачи и примеры JSON зафиксированы для текущего `develop`. При изменении контракта frontend и backend обновляются согласованно. Дизайн-токены и графические материалы подключаются к общей теме интерфейса после подготовки брендбука. Каждая новая вертикальная часть должна оставаться запускаемой и проверяемой до слияния.
