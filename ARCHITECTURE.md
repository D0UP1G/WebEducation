...
Статус: архитектура MVP включает изолированный Python-runner (PR #50) и импорт
пакета организатора с сохранением модулей и паспортов курса (PR #51). Локальный
Compose проверен; публичный стенд и полный E2E трёх ролей остаются открытыми.
Оставшиеся задачи — в [аудите от 25.09](docs/case-compliance-and-task-plan-2026-09-25.md).
...
Для демонстрации есть синтетический курс и синтетические учётные записи. [Базовый учебный пакет и официальный брендбук](docs/organizer/README.md) размещены в проекте; команда импорта сохраняет три курса, девять модулей и 30 шагов в БД и публикует версии. Исходный DOCX с ответами опубликован в открытом Git по решению владельца. Текущее соответствие кейсу отражено в [сверке](docs/current-gap-analysis.md).
...
| Сущность | Назначение |
|---|---|
| `User` | Пользователь, роль и отображаемое имя; удаление обезличивает запись и сохраняет ссылки на учебную историю |
| `Course` | Название, описание, диапазон классов, владелец, `source_id`, паспорт курса (`tool`, `goal`, `volume`), баннер и флаг архива |
| `Module` | Группировка черновых шагов внутри курса с позицией и стабильным `source_id` |
| `CourseRevision` | Неизменяемый опубликованный выпуск курса вместе с копией паспорта, баннера и ключа источника |
| `ModuleRevision` | Неизменяемый модуль конкретного выпуска курса |
| `StepRevision` | Шаг конкретной версии: ключ типа, порядок, контент, настройки, баллы и ссылка на модуль версии |
...
| `StepQuestion` | Переписка ученика с закреплённым куратором по шагу в конкретном назначении |
| `StepQuestionMessage` | Одно сообщение ученика или куратора внутри переписки |

Публикация создаёт новую версию курса. Уже начатые записи остаются привязаны к прежней версии, чтобы прошлые попытки и результаты не меняли смысл. Новые назначения получают последнюю опубликованную версию. Перевод текущей записи на новую версию можно добавить отдельно, если это подтвердят организаторы. Опубликованный курс нельзя физически удалить: его архивируют, снимая возможность новых назначений; снятая запись `Enrollment` получает статус `removed`, скрывается у ученика, но её попытки и переписка сохраняются. Черновой курс без опубликованных ревизий можно удалить.

Шаг открывается только когда каждый предыдущий шаг имеет принятую сдачу. То же правило серверно применяется к чтению содержимого, примеру Python, вопросам и отправке работы; одного сокрытия в React недостаточно. Ожидание ручной проверки блокирует следующий шаг до решения куратора. Это сознательное продуктовое решение владельца, несмотря на иллюстрацию в §06 брендбука, где во время ручной проверки разрешён следующий шаг.

Прогресс выводится из принятых шагов, а не хранится как независимое число, которое может разойтись с попытками. Баллы за принятый шаг начисляются один раз независимо от того, проверил его алгоритм или куратор. В первом рабочем срезе принятый шаг даёт полные `max_score`, остальные попытки — ноль. `completion_percent = floor(100 × принятые шаги / все шаги)`, `rating_percent = floor(100 × заработанные баллы / доступные баллы)`. Публикация пустого курса или курса с нулевой суммой баллов запрещена. Фронтенд показывает обе величины и разбивку по шагам; ученический профиль агрегирует эти показатели из уже существующего прогресса. Правила подробно зафиксированы в [сверке с кейсом](docs/case-alignment.md).
...
6. Создаёт учётные записи учеников/кураторов без начального пароля и передаёт одноразовую ссылку для установки пароля; повторная ссылка сбрасывает прежний пароль. Удаление пользователя обезличивает запись, не каскадно удаляя работы и назначенные курсы.
7. Удаляет неопубликованный черновик или архивирует курс с опубликованной историей; в назначениях можно снять курс и сохранить связанные данные.
...
6. Может продолжить переписку с куратором в этом шаге. Черновик кода Python хранится локально в браузере отдельно для каждого шага.

### Куратор

1. Видит только закреплённых за ним учеников.
2. Открывает очередь ручных проверок.
3. Принимает работу или возвращает её с обязательным комментарием; комментарий виден ученику в обоих случаях.
4. Продолжает переписки по шагам и видит объяснимые признаки застоя: 72 часа без зачёта, две неверные попытки на одном шаге за 24 часа или возврат без пересдачи 24 часа. Эти пороги — начальная гипотеза для демо.

Статусы сдачи: `queued`, `checking`, `pending_review`, `accepted`, `incorrect`, `returned`, `error`. `incorrect` означает неуспешную автоматическую проверку, `returned` — решение куратора, `error` — технический сбой. Повторная сдача создаёт новую попытку; история старых попыток и отзывов остаётся доступной. На шаге с активной проверкой повторная сдача блокируется. После зачёта повторные серверные попытки доступны для `algorithm.python`, чтобы ученик мог проверять другие варианты кода; любой предыдущий принятый вариант сохраняет шаг зачтённым, а баллы начисляются в прогрессе один раз. Остальные принятые типы шагов повторно сдать нельзя.

## 9. API-контракт

Все маршруты имеют префикс `/api/v1/`; актуальные endpoint, форматы ответов,
статусы и правила доступа описаны в [`docs/api-contract.md`](docs/api-contract.md).
Фронтенд и бэкенд должны обновлять контракт при изменении поведения.

Основные группы маршрутов:

| Контур | Примеры операций |
|---|---|
| Авторизация | Вход, выход, текущая сессия, одноразовый `POST /auth/set-password` |
| Ученик | Каталог, курс/прогресс, шаги, сдачи, открытый пример Python, переписки и продолжение сообщений |
| Куратор | Ученики, очередь и решения с обязательным комментарием, переписки и сообщения |
| Администратор | Курсы/публикация/архив, пользователи/ссылки для пароля, групповые назначения и фильтры |

Каждый эндпоинт проверяет роль и связь пользователя с курсом на сервере. Снятые
назначения и закрытые шаги не открывают ученический контент; точные ответы и
закрытые тесты не выдаются ученическому API.

## 10. Запуск кода и загруженные файлы

Проверка Python-кода — обязательный сценарий кейса. Реализация в `develop`
...
`import_curriculum` создаёт или обновляет пакет по стабильным `source_id`, а
публикация сохраняет структуру модулей и шагов в неизменяемых ревизиях. При
изменении контракта frontend и backend обновляются согласованно. Каждая новая
вертикальная часть должна оставаться запускаемой и проверяемой до слияния.
