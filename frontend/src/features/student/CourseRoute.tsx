...
function StepIcon({ type }: { type: StepType }) {
  const path = type === 'theory'
    ...
    : type.startsWith('quiz.') || type === 'answer.exact' || type === 'scratch.numeric_answer'
  ...
  return <svg viewBox="0 0 20 20" width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{path}</svg>
}
...
export function CourseRoute({ detail, compact = false, currentStepId }: {
  ...
  if (compact) return <aside className="card route-aside" aria-label="Шаги курса">
    ...
    <ol className="mini-route">
      {sorted.map((step) => <li key={step.id} className={step.id === currentStepId ? 'is-current' : ''}>
  ...
  return <section className="card course-route-card" aria-labelledby="course-route-title">
    ...
    <ol className="course-route">
      {sorted.map((step) => {
        ...
        return <li key={step.id} className={current ? 'is-current' : status === 'accepted' ? 'is-done' : ''}>
    ...
    {detail.progress.next_action === 'await_review' && <p className="route-note">Куратор проверяет работу. Твой результат обновится после проверки.</p>}
  </section>
}
